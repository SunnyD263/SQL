...
Declare @MinID datetime, @MaxID datetime,@DT date, @Zsum int = 0,@X int = 0, @Y int = 0,@Z int=0, @Xsum int = 0, @Ysum int = 0,
@StDate datetime, @NdDate datetime, @User varchar(30), @EndTime time(0)
	
update [dbo].[Databuff] set [ActTime]= CONVERT(time(0),  '00:00:00')


set @Zsum = (select count(*) from (SELECt cast(dati as date) as DT from Databuff  group by cast(dati as date)) as D)
WHILE (@Zsum <> @Z )
Begin

if (@Z=0) CREATE TABLE #Temp(RowID INT PRIMARY KEY IDENTITY(1,1), DT datetime)
INSERT INTO #temp(DT) SELECT E.DT from (SELECt cast(dati as date) as DT from Databuff where  ActTime = CONVERT(time(0),  '00:00:00' )  group by cast(dati as date)) as E;
			WITH MyCte AS 
			(SELECT DT , RowID from #temp T1 JOIN (SELECt cast(dati as date) as ZZZ from Databuff where ActTime = CONVERT(time(0),  '00:00:00' )  group by cast(dati as date)) T2 ON T1.DT = T2.ZZZ) --WHERE T1.RowID = @Y+1
			SELECT  @DT = DT
			FROM    MyCte
			WHERE   RowID = @Z + 1
			...
			WHERE   RowID = @Y + 1
				...
				where cast(dati as date) = @DT and ActTime = CONVERT(time(0),  '00:00:00') and userid = @User )
				WHILE (@Xsum <> @X )
				
				
				Begin
				set @MinID = (select  MIN(DaTi) from Databuff where cast(dati as date) = @DT and ActTime = CONVERT(time(0),  '00:00:00') and userid = @User)
				set @MaxID = (select  Min(DaTi) from Databuff where cast(dati as date) = @DT and ActTime = CONVERT(time(0),  '00:00:00') and userid = @User and DaTi <> @MinID )
				...
				update [dbo].[Databuff] set [ActTime] = @EndTime where DaTi = @MinID and  userid = @User
				if (@Xsum = 1) update [dbo].[Databuff] set [ActTime] = Null where DaTi = @MinID and  userid = @User;
				Set @X = @X + 1
				IF (@Xsum = @X) BREAK;
				End
			set @MinID = null
